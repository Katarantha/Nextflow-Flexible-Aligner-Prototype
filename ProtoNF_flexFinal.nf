...
log.info """\
    ...
    P R O T O N F _ flexHisat2 PIPELINE
    ========================
    followup to ProtoNF_flexAlign prototype that incorporates the optional usage of a STAR and Hisat2 aligners in addition to the existing novoalign functionality
    ========================
    Reads                   : ${params.reads}
...
process BOWTIE2_INDEX{
    ...
    script:
    """
    bowtie2-build -f '$genome' 'Bowtie2_index'
    """
...
process HISAT2{
    ...
    publishDir './results/AlignmentHisat2' , mode: 'copy', overwrite: false
    ...
    script:
    """
    hisat2 -x 'hisat2_index' -U '$collapsed_files' -f -S '${collapsed_files.baseName}_hisat2-aligned.bam'
    """
...
process BOWTIE2{
    ...
    publishDir './results/AlignmentBowtie2' , mode: 'copy', overwrite: false
    ...
    script:
    """
    bowtie2 -x 'Bowtie2_index' -U '$collapsed_files' -S 'Bowtie2_Aligned_${collapsed_files.baseName}.bam' -f
    """
...
workflow {
    ...
    if (params.aligner == 'hisat2'){
        indexed_files_ch = HISAT2_INDEX( genome_ch )
    }

    if (params.aligner == 'bowtie2'){
        indexed_files_ch = BOWTIE2_INDEX( genome_ch )
    }
    ...
    if (params.aligner == 'hisat2'){
        align_ch = HISAT2( indexed_files_ch, collapse_ch)
    }

    if (params.aligner == 'bowtie2'){
        align_ch = BOWTIE2( indexed_files_ch, collapse_ch)
    }
